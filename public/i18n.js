(function () {
  ...
  const translations = {
    ...
    '企業・学校・団体などで活用いただいた際は、企業・団体名と、公開可能な利用風景や用途をお送りください。許可いただいた内容をSONA STUDIOの活用事例として紹介させていただく場合があります。':'If your company, school, or organization uses QRtoQR, send us your organization name and any publishable photos or usage details. With permission, we may feature them as a SONA STUDIO use case.',
    ...
    '本規約は、SONA STUDIOが提供する「QRtoQR」の利用条件を定めるものです。':'These Terms set out the conditions for using QRtoQR, provided by SONA STUDIO.',
    ...
    'Google Spreadsheetを選択':'Select Google Spreadsheet','Googleアカウントに接続されていません。':'Not connected to a Google Account.','または公開URLから読み込む':'Or load from a public URL','上書き先のGoogle Spreadsheetを選択':'Select a Google Spreadsheet to overwrite','Spreadsheetが選択されていません。':'No Spreadsheet selected.','Googleアカウントで接続すると、選択したSpreadsheetへ上書きできます。':'Connect your Google Account to overwrite the selected Spreadsheet.',
    '画面右上の「データインポート」からファイルを選ぶか、Googleアカウントに接続してSpreadsheetを選択します。公開SpreadsheetはURLからも読み込めます。':'Choose a file from Import Data, or connect your Google Account and select a Spreadsheet. Public Spreadsheets can also be loaded by URL.','既存のキーを読み取ると、出力画面左上にデータ入力への切替ボタンが表示されます。左右のスワイプで列を選び、カメラで読み取った文字列を空白セルへ追加できます。空白列ではカメラボタンから現在の映像をPNG画像として保存し、キーと撮影日時から作成したファイル名をセルへ登録します。入力済みセルは上書きまたは削除できます。Google Spreadsheetから読み込んだ場合は、エクスポート画面から変更内容を書き戻せます。':'After scanning an existing key, use the upper-left button to switch to data input. Swipe to choose a column and scan text into a blank cell. The camera button saves a PNG and records a filename made from the key and capture time. Existing cells can be overwritten or deleted. Changes can be written back from Export when using Google Spreadsheet.','非公開SpreadsheetはGoogleアカウントに接続して選択してください。URL読み込みを使う場合は公開設定が必要です。':'Connect your Google Account to select a private Spreadsheet. URL loading requires public sharing.','Googleアカウント連携を選択した場合、Googleの認証画面とPickerを使用し、利用者が選択したSpreadsheetだけを読み書きします。アクセストークンと取得データはブラウザ内で処理し、本アプリのサーバーには保存しません。公開URLから読み込む場合は、ブラウザがGoogleへ直接アクセスして公開CSVを取得します。入力した公開URLは次回利用時のためブラウザ内に保存します。Google側での取り扱いはGoogleの規約とポリシーに従います。':'When Google Account integration is used, Google authorization and Picker are used to read and write only the Spreadsheet selected by the user. Access tokens and retrieved data are processed in the browser and are not stored on our server. For a public URL, the browser retrieves the public CSV directly from Google and stores the entered URL locally for future use. Google handles related data under its terms and policies.'
  };
