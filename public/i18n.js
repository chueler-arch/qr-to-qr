(function () {
  ...
  const translations = {
    ...
    'データ管理':'Data Management','インポート':'Import','エクスポート':'Export','CSVファイルをエクスポート':'Export CSV File','Google Apps Script WebアプリURL':'Google Apps Script Web App URL','共有トークン':'Shared Token','Spreadsheetを上書き':'Overwrite Spreadsheet','Spreadsheetへの書き込みには、専用Apps Scriptのデプロイが必要です。':'Writing to Spreadsheet requires the included Apps Script web app.','データエクスポート':'Export Data',
    'Spreadsheetに上書きする場合には、共有トークンが必要です。':'A shared token is required to overwrite the Spreadsheet.','共有トークンの取得手順':'How to Get a Shared Token','Google Apps ScriptでQRtoQR用のプロジェクトを開きます。':'Open the QRtoQR project in Google Apps Script.','「プロジェクトの設定」からスクリプト プロパティを開きます。':'Open Script Properties from Project Settings.','プロパティ名を QRTOQR_TOKEN とし、推測されにくい長い文字列を値に設定します。':'Set the property name to QRTOQR_TOKEN and use a long, hard-to-guess value.','同じ文字列をこの画面の共有トークン欄へ入力します。':'Enter the same value in the Shared Token field.','トークンは第三者に公開しないでください。QRtoQRではこのブラウザ内に保存されます。':'Do not share the token. QRtoQR stores it in this browser.','上書き':'Overwrite','押している間だけ上書きが有効となります':'Overwrite is enabled only while holding the button.'
  };
