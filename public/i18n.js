(function () {
  ...
  const translations = {
    ...
    '上書き':'Overwrite','押している間だけ、バーコードを上書き入力します。':'While held, the scanned barcode replaces the current value.','セルを編集':'Edit Cell','セルの値':'Cell value','テキストの入力・編集・削除ができます。':'You can enter, edit, or delete text.','保存':'Save',
    ...
    '画面右上の「データインポート」からファイルを選ぶか、Googleアカウントに接続してSpreadsheetを選択します。公開SpreadsheetはURLからも読み込めます。':'Choose a file from Import Data, or connect your Google Account and select a Spreadsheet. Public Spreadsheets can also be loaded by URL.','既存のキーを読み取ると、出力画面左上にデータ入力への切替ボタンが表示されます。左右のスワイプで列を選び、カメラで読み取った文字列を空白セルへ追加できます。空白列ではカメラボタンから現在の映像をPNG画像として保存し、キーと撮影日時から作成したファイル名をセルへ登録します。入力済みセルは上書きまたは削除できます。Google Spreadsheetから読み込んだ場合は、変更したセルだけが約1秒後に自動保存されます。':'After scanning an existing key, use the upper-left button to switch to data input. Swipe to choose a column and scan text into a blank cell. The camera button saves a PNG and records a filename made from the key and capture time. Existing cells can be overwritten or deleted. For Google Spreadsheet imports, only changed cells are saved automatically after about one second.','非公開SpreadsheetはGoogleアカウントに接続して選択してください。URL読み込みを使う場合は公開設定が必要です。':'Connect your Google Account to select a private Spreadsheet. URL loading requires public sharing.','Google Spreadsheet・Google Drive':'Google Spreadsheet and Google Drive','Googleアカウント連携を選択した場合、Googleの認証画面とPickerを使用し、利用者が選択したSpreadsheetだけを読み書きします。追加モードで撮影した画像は、利用者がGoogleアカウントに接続している場合に限り、その利用者のGoogle Driveへ保存し、閲覧リンクをSpreadsheetの対象セルへ記録します。本アプリが作成した画像以外のDriveファイルを変更することはありません。アクセストークンと取得データはブラウザ内で処理し、本アプリのサーバーには保存しません。公開URLから読み込む場合は、ブラウザがGoogleへ直接アクセスして公開CSVを取得します。入力した公開URLは次回利用時のためブラウザ内に保存します。Google側での取り扱いはGoogleの規約とポリシーに従います。':'When Google Account integration is used, Google authorization and Picker read and update only the Spreadsheet selected by the user. Images captured in Add Mode are saved to the connected user’s Google Drive, and their viewing links are recorded in the target Spreadsheet cells. The app does not modify Drive files other than images it creates. Access tokens and retrieved data are processed in the browser and are not stored on our server. For a public URL, the browser retrieves the public CSV directly from Google and stores the entered URL locally for future use. Google handles related data under its terms and policies.',
    'Google Driveから直接取り込む':'Import directly from Google Drive','Googleアカウントに接続してSpreadsheetを選択します':'Connect your Google Account and select a Spreadsheet','ファイルを選択する':'Choose a file','端末内のファイル、または公開Spreadsheetを読み込みます':'Load a local file or a public Spreadsheet','GoogleスプレッドシートURLを指定':'Specify a Google Spreadsheet URL','公開設定とリンクURLの取得手順':'How to publish and copy the link URL','共有で公開設定したGoogle SpreadsheetのリンクURLを貼ってください。':'Paste the link URL of a Google Spreadsheet made public through its sharing settings.','Google Spreadsheetを開き、右上の「共有」を押します。':'Open the Google Spreadsheet and click Share in the upper right.','「一般的なアクセス」を「リンクを知っている全員」に変更します。':'Change General access to Anyone with the link.','権限は「閲覧者」を選択します。':'Choose Viewer permission.','「リンクをコピー」を押し、上の入力欄へ貼り付けます。':'Click Copy link and paste it into the field above.','公開URL方式では閲覧のみ可能です。直接書き込みする場合は「Google Driveから直接取り込む」を使用してください。':'Public URL import is read-only. To write changes directly, use Import directly from Google Drive.','この方法ではGoogle Spreadsheetへの書き込みはできません。追加・編集したデータは、エクスポートするCSVファイルにのみ反映されます。':'This method cannot write to Google Spreadsheet. Added or edited data is reflected only in the exported CSV file.','読み込む':'Load','データを読み込んでいます':'Loading data'
  };
  ...
  function translateTree() {
    ...
    document.querySelectorAll('.language-toggle').forEach((button) => { button.textContent = language === 'en' ? 'JP' : 'EN'; button.setAttribute('aria-label', language === 'en' ? '日本語に切り替え' : 'Switch to English'); });
    if (language === 'en') {
