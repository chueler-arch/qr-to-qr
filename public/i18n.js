(function () {
  ...
  const translations = {
    ...
    'Google Driveから直接取り込む':'Import directly from Google Drive','Googleアカウントに接続してSpreadsheetを選択します':'Connect your Google Account and select a Spreadsheet','ファイルを選択する':'Choose a file','端末内のファイル、または公開Spreadsheetを読み込みます':'Load a local file or a public Spreadsheet','公開設定とリンクURLの取得手順':'How to publish and copy the link URL','Google SpreadsheetのリンクURL':'Google Spreadsheet link URL','Google Spreadsheetを開き、右上の「共有」を押します。':'Open the Google Spreadsheet and click Share in the upper right.','「一般的なアクセス」を「リンクを知っている全員」に変更します。':'Change General access to Anyone with the link.','権限は「閲覧者」を選択します。':'Choose Viewer permission.','「リンクをコピー」を押し、下の入力欄へ貼り付けます。':'Click Copy link and paste it into the field below.','公開URL方式では閲覧のみ可能です。直接書き込みする場合は「Google Driveから直接取り込む」を使用してください。':'Public URL import is read-only. To write changes directly, use Import directly from Google Drive.','データを読み込んでいます':'Loading data'
  };
